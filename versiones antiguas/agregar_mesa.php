...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$numero_mesa || !$capacidad_mesa) {
        header('Location: inicio.php?msg=❌ Debes completar todos los campos.');
        exit;
    }

    // Validar que sean números
    if (!is_numeric($numero_mesa) || !is_numeric($capacidad_mesa)) {
        header('Location: inicio.php?msg=❌ El número de mesa y capacidad deben ser números.');
        exit;
    }
    ...
    if ($resultCheck->num_rows > 0) {
        header('Location: inicio.php?msg=❌ La mesa número ' . $numero_mesa . ' ya existe.');
        exit;
    }
    ...
    if ($stmt->execute()) {
        header('Location: inicio.php?msg=✅ Mesa número ' . $numero_mesa . ' agregada exitosamente.');
        exit;
    } else {
        header('Location: inicio.php?msg=❌ Error al agregar la mesa: ' . $stmt->error);
        exit;
    }
