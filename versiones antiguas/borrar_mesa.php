...
if (isset($_GET['id'])) {
    ...
    if ($resultCheck->num_rows === 0) {
        header('Location: inicio.php?msg=❌ La mesa no existe.');
        exit;
    }
    ...
    if ($mesa['estado_mesa'] !== 'libre') {
        header('Location: inicio.php?msg=❌ Solo se pueden borrar mesas en estado libre.');
        exit;
    }
    ...
    try {
        ...
        header('Location: inicio.php?msg=✅ Mesa eliminada y números reorganizados.');
        exit;
    } catch (Exception $e) {
        // Revertir transacción en caso de error
        $conn->rollback();
        header('Location: inicio.php?msg=❌ Error al eliminar la mesa: ' . $e->getMessage());
        exit;
    }
